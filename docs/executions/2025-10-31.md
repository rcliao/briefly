...
### Completed
- ✅ Kagi News research and design document creation
- ✅ v2.1 design document with all editorial changes
- ✅ Split design into modular documents (Product, Inspiration, Architecture, Execution)

### Completed (2025-10-31)

#### 1. LangFuse Setup ✅
- ✅ Implemented simplified LangFuse client with local logging (`internal/observability/langfuse.go`)
- ✅ Trace and generation tracking interfaces defined
- ✅ TracedClient wraps LLM client with observability (`internal/llm/traced_client.go`)
- ✅ Cost estimation for Gemini models
- ⚠️  Note: Using local logging approach (not full SDK integration - HTTP API can be added later)

**Status:** Core functionality complete, ready for HTTP API integration when LangFuse SDK stabilizes

#### 2. PostHog Setup ✅
- ✅ Installed PostHog Go SDK (`github.com/posthog/posthog-go`)
- ✅ Created PostHog client (`internal/observability/posthog.go`)
- ✅ Pre-built tracking methods for all key events:
...
- ✅ Frontend PostHog tracking in web pages (`/themes`, `/submit`)
- ✅ Configuration via environment variables (POSTHOG_API_KEY, POSTHOG_HOST)

**Status:** Fully integrated, tracking all key user and system events

#### 3. Theme System ✅
- ✅ Created `themes` table with migration (`003_add_themes.sql`)
- ✅ Implemented `ThemeRepository` with full CRUD (`internal/persistence/postgres_repos.go`)
- ✅ Built `LLMThemeClassifier` with JSON-based prompts (`internal/themes/classifier.go`)
- ✅ Theme categorizer integrated into digest pipeline (`internal/pipeline/theme_categorizer.go`)
...
- ✅ Seeded 10 default themes (`006_seed_default_themes.sql`):
  - AI & Machine Learning
  ...
  - Product & Startup
- ✅ Web API endpoints (`/api/themes`)
- ✅ Web management page (`/themes`)
- ⚠️  Unit tests pending (will test after full vertical slice)

**Status:** Fully implemented, integrated into pipeline with 40% relevance threshold

#### 4. Manual URL Submission ✅
- ✅ Created `manual_urls` table with migration (`004_add_manual_urls.sql`)
- ✅ Implemented `ManualURLRepository` with status tracking
- ✅ Added `article_themes` table link (`005_add_article_themes.sql`)
- ✅ CLI commands implemented:
...
- ✅ Web API endpoints (`/api/manual-urls`)
- ✅ Web submission page (`/submit`)
- ✅ Integration with aggregation pipeline (`sources.Manager.AggregateManualURLs`)
- ✅ Automatic processing in `briefly aggregate` command
...
- ✅ Updated pipeline builder to support theme-based categorization
- ✅ PostHog tracking in all web pages (themes, submit)
- ✅ Graceful fallback from theme categorization to legacy categories
- ✅ Database-driven theme system with LLM classification
- ✅ Builder pattern for optional theme system integration
...
### Deliverables
- ✅ LangFuse tracking all LLM operations
- ✅ PostHog tracking basic page views
- ✅ Working theme classification system
- ✅ Manual URL submission (CLI + Web)
...
#### 4. Theme Classification with Structured Output ✅ (0.5 days - Priority 0)
- ✅ Migrated theme classification to use Gemini `response_schema` API
...
- ✅ Fixed critical bugs in `cmd/handlers/classify.go`:
...
- ✅ Tested with 20 articles: 17/20 classified successfully (85% success rate)
...
#### 3. Citation Tracking ✅ (1 day - Priority 1)
- ✅ Created `citations` table with migration (`008_add_citations.sql`)
- ✅ Implemented `CitationRepository` interface with CRUD operations
- ✅ Implemented `postgresCitationRepo` with PostgreSQL backend
- ✅ Created `CitationTracker` component (`internal/citations/tracker.go`)
...
- ✅ Integrated into article processing pipeline
...
#### 2. Structured Summary Generation ✅ (3 days - Priority 2)
- ✅ Updated `summaries` table schema (`009_add_structured_summaries.sql`)
  - Added `summary_type` column ("simple" | "structured")
  - Added `structured_content` JSONB column
  - Created indexes for efficient querying
- ✅ Defined `StructuredSummaryContent` in `internal/core/core.go`:
  ```go
  ...
  ```
- ✅ Implemented `StructuredSummarizer` using Gemini `response_schema` API
...
- ✅ **Section-level LangFuse tracking** (`TracedSummarizer`)
...
**Goal:** Working showcase with RSS → Classified Articles → Digest → Web Viewer → Deploy
...
#### 2. Digest Generation from Database (1 day)
- [ ] Create new command: `briefly digest generate [--theme X] [--since 7]`
  - [ ] Query `articles` table (filter by theme, date range)
  - [ ] Convert DB articles → pipeline format
  - [ ] Run through existing 9-step pipeline (clustering, summarization)
  - [ ] Store in `digests` table
  - [ ] Output markdown file
- [ ] Update tests

**Acceptance Criteria:**
- `briefly digest generate --theme "AI & Machine Learning" --since 7` works
- Digest generated from classified articles in database
- Clustering and executive summary working
- Markdown output saved to digests directory

#### 3. Web Digest Viewer (1 day)
- [ ] Create homepage (`/`) - show latest digest
- [ ] Create digest viewer (`/digests/{date}`) - structured display
- [ ] Basic styling (TailwindCSS CDN)
- [ ] Article cards with citations
- [ ] Theme filtering (if multiple themes)
- [ ] PostHog tracking for page views

**Acceptance Criteria:**
- Beautiful digest page at `http://localhost:8080/`
- Articles displayed with structured summaries
- Citations visible and formatted
...
- ✅ **Citations tracked and retrievable** (4 formats: Simple, APA, MLA, Chicago)
- ✅ **Structured summaries with 5 sections** (Key Points, Context, Main Insight, Technical Details, Impact)
- ✅ **Section-level LangFuse tracking** for observability
- ✅ **Theme classification with structured output** (production-ready)
- ⚪ **RSS feeds automatically classified during aggregation** (In Progress)
- ⚪ **Digest generation from database articles** (Pending)
- ⚪ **Web digest viewer** (Pending)
- ⚪ **Production deployment** (Pending)

### Summary
**Phase 1 Status: 75% Complete** (3 of 4 priorities done)
- Core features complete: Citations, Structured Summaries, Theme Classification
- This week: Complete aggregation, digest generation, web viewer, deploy
...
- ✅ Search queries configurable via CLI/Web
- ✅ Search results classified and filtered by theme
- ✅ All search operations tracked in LangFuse
...
### Deliverables
- ✅ Working pgvector similarity search
- ✅ RAG context used in summarization and classification
- ✅ Measurable improvement in consistency (eval scores)
...
- ✅ 100+ labeled examples across tasks
- ✅ Ability to compare prompts and track improvements
- ✅ Evaluation runs linked to LangFuse
...
- ✅ Public can view digests with theme filtering
- ✅ Admin can manage all sources and configurations
- ✅ PostHog tracking user engagement
...
### Deliverables
- ✅ All pipeline steps runnable manually via CLI
- ✅ Choice of in-app or infrastructure-level CRON
...
### Deliverables
- ✅ Production deployment at public URL
- ✅ Complete documentation
- ✅ LangFuse and PostHog dashboards live
- ✅ Performance optimized for 100+ articles/week
...
|-------|----------|------------------|--------|
| Phase 0 | Week 1 (Oct 31 - Nov 6) | LangFuse, PostHog, Themes, Manual URLs | ✅ Complete |
| Phase 1 | Week 2 (Nov 3 Morning) | Citations, Structured Summaries, Theme Classification | ✅ Complete |
| Phase 1 (This Week) | Nov 3-8 | Inline Aggregation, Digest from DB, Web Viewer, Deploy | 🟢 In Progress |
| Phase 2 | Deferred | Search Integration (Optional) | ⚪ Deferred |
| Phase 3 | Deferred | RAG with pgvector | ⚪ Deferred |
| Phase 4 | Deferred | Evaluation Framework | ⚪ Deferred |
| Phase 5 | Deferred | Full Admin Dashboard | ⚪ Deferred |
| Phase 6 | Deferred | Automation + CRON | ⚪ Deferred |
| Phase 7 | Nov 8 | Minimal Deployment + Polish | 🟢 This Week |
...
- ✅ RSS feeds with theme filtering
...
1. ✅ Complete design document split (Product, Inspiration, Architecture, Execution)
