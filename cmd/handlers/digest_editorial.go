...
func renderEditorialDigest(d *editorialDigest, articles []core.Article, failed []failedLink, generatedAt time.Time) string {
	...
	if d.MustRead != nil {
		...
		fmt.Fprintf(&b, "🎯 Must-read: %s — %s 📖 %d min\n\n%s\n\n",
	...
	if len(failed) > 0 {
		fmt.Fprintf(&b, "⚠️ %d link(s) could not be included:\n", len(failed))
	...
	fmt.Fprintf(&b, "Generated by Briefly on %s\n", generatedAt.Format("2006-01-02"))
